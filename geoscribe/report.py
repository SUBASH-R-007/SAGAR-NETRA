...
def priority_for(severity: float) -> str:
    """Triage priority from the severity bands: HIGH in the critical band
...
def _summary_block(
    contacts: list[Contact], survey_stats: dict[str, Any] | None
) -> dict[str, Any]:
    """Survey-level roll-up for contacts.json (strategy-PDF section 5.3).

    Coverage numbers come from ``survey_stats`` (see
    :func:`geoscribe.build.survey_stats`); without them — old callers, fast
    preview passes — the counts still stand and the coverage keys are null,
    which the published schema allows.
    """
    summary: dict[str, Any] = {
        "total_detections": len(contacts),
        "high_confidence": sum(1 for c in contacts if c.confidence >= HIGH_CONFIDENCE_PCT),
        "area_surveyed_sqkm": None,
        "debris_density_per_sqkm": None,
        "sonar_config": None,
    }
    if survey_stats:
        area = survey_stats.get("area_surveyed_sqkm")
        summary["area_surveyed_sqkm"] = area
...
def write_contacts_json(
    contacts: list[Contact],
    path: str | Path,
    survey: str,
    survey_stats: dict[str, Any] | None = None,
) -> Path:
    path = Path(path)
    ...
    doc = {
        ...
        "summary": _summary_block(contacts, survey_stats),
...
def write_contacts_csv(contacts: list[Contact], path: str | Path) -> Path:
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    columns = [
        "id", "class", "confidence_pct", "severity", "priority", "recommended_action",
    ...
    with path.open("w", newline="", encoding="utf-8") as fh:
        ...
        for c in contacts:
            writer.writerow([
                c.id, c.cls, c.confidence, c.severity, c.priority, c.recommended_action,
                c.lat, c.lon, c.position_accuracy_m,
...
def write_report_pdf(
    ...
    # Summary table.
    rows = [["ID", "Class", "Conf %", "Severity", "Priority", "Lat", "Lon", "L×W (m)", "H (m)"]]
    for c in contacts:
        rows.append([
            c.id, c.cls, f"{c.confidence:.0f}", f"{c.severity:.0f}", c.priority,
...
def write_all(
    ...
    ``survey_stats`` (see :func:`geoscribe.build.survey_stats`) feeds the
    contacts.json summary block; None leaves its coverage numbers null.
    """
    out_dir = Path(out_dir)
    return {
        "json": write_contacts_json(
            contacts, out_dir / "contacts.json", survey, survey_stats=survey_stats
        ),
